...
Return ONLY valid JSON, no markdown or explanation.`;
  ...
  try {
    ...
    console.error(
      "[Dating Studio] Failed to parse reference analysis:",
      cleanText
    );
    return {
...
function buildDatingPrompt(
  ...
  const prompt = `Generate a professional dating profile photo of a ${
    face.ageRange
  } ${face.gender} with ${face.hairColor} ${face.hairStyle} hair and ${
    face.skinTone
  } skin. ${face.facialFeatures}. ${
    face.distinctiveFeatures ? face.distinctiveFeatures + "." : ""
  }
...
Only approve if identityScore >= 85 AND isEcho is false.`;
    ...
    if (jsonMatch) {
      ...
      return {
        ...
        reason:
          data.reason ||
          (data.isEcho ? "Detected reference echo" : "Identity mismatch"),
      };
    }
  } catch (error) {
...
- Authentic, raw, high-resolution photography only.`;
    ...
    console.log(
      "[Dating Studio] Generating with Gemini 3 Pro Image (Identity Lock + Descriptors)..."
    );
    ...
    if (textPart && "text" in textPart) {
      console.log(
        "[Dating Studio] Got text response instead of image:",
        textPart.text
      );
      return {
        success: false,
        error: `Model returned text: ${(
          textPart as { text: string }
...
export async function generateDatingProfilePhotos(
  ...
  console.log(
    `[Dating Studio] Starting generation: ${selfies.length} selfies, ${references.length} unique references requested`
  );
  ...
  if (references.length === 0) {
    return {
      success: false,
      imageUrls: [],
      errors: ["No reference images provided"],
    };
  }
  ...
  try {
    ...
    console.warn(
      "[Dating Studio] Face analysis failed, continuing without descriptors"
    );
  }
  ...
  async function generateWithRetries(index: number): Promise<string | null> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      console.log(
        `[Dating Studio] Slot ${index + 1} - Attempt ${
          attempt + 1
        }/${maxRetries}`
      );
      ...
      try {
        ...
        if (result.success && result.imageBase64) {
          ...
          console.log(
            `[Dating Studio] Slot ${
              index + 1
            } generated successfully (Identity Score: ${
              validation.identityScore
            })`
          );
  ...
  const generationPromises = Array.from({ length: maxPhotos }).map((_, i) =>
    generateWithRetries(i)
  );
